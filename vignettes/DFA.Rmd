...
Compare three states models with R diagonal and unequal and 12 time series. The EM algorithm struggles to converge with DFA models.
```{r}
...
t1.bfgs <- system.time(m1.bfgs <- MARSS(dat2, model=mod.list, form='dfa', z.score=FALSE, silent = TRUE, method="BFGS"))[1]
...
TMB is much faster. But at the default convergence settings, BFGS ran longer and got to a higher maximum likelihood.
```{r}
c(m1.em$logLik, m1.bfgs$logLik, m1.tmb$logLik)
c(em=t1.em, bfgs=t1.bfgs, tmb=t1.tmb)
```

It might be an odd initial condition issue as if BFGS is started with EM, it does not find the lower log-likelihood but ends up at the higher value that EM and TMB found.
```{r}
