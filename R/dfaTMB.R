...
#' @export
dfaTMB <- function(obs, NumStates=1, ErrStruc='DE', EstCovar=FALSE,
                Covars=NULL, indivCovar=FALSE, Dmat=NULL, Dfac=NULL,
                Rfac=NULL, logsdObs=NULL, logsdObsFac=NULL, 
                cholCorr=NULL, cholFac=NULL, EstSE=FALSE,
                silent=TRUE){ 
  obs<-t(obs)
  ...
  if(EstCovar){ # If you are estimating covariates this creates Dmat and Dfac based on the data, number of covars
    if(is.null(Dmat) & is.null(Dfac)){ #This checks that you did not supply Dmat or Dfac (Manual covariate parameter entries)
      if(!indivCovar){
